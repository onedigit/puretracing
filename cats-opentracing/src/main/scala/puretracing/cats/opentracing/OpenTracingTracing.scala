...
import io.opentracing.propagation.{Format, TextMap, TextMapExtractAdapter, TextMapInjectAdapter}
...
class OpenTracingTracing[F[_]](tracer: io.opentracing.Tracer)(implicit F: Sync[F]) extends Tracer[F] {
  ...
  override def startRootSpan(operationName: String, upstreamSpan: Headers): F[Span] = {
    ...
    /*
    for {
      upstream <- F.catchNonFatal(
        tracer
        // TODO
        // this doesn't compile
          .extract(Format.Builtin.HTTP_HEADERS, new TextMapExtractAdapter(upstreamSpan.asJava))
      )
    ...
    for {
      _ <- F.catchNonFatal(
        tracer
          // TODO
          // this doesn't compile
          .inject(span.context, Format.Builtin.HTTP_HEADERS, new TextMapInjectAdapter(carrier))
      )
  ...
  override def setBaggageItem(span: Span, key: String, value: String): F[Unit] =
    F.delay(span.setBaggageItem(key, value))
