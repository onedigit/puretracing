...
class ReaderTPropagation[F[_]: Applicative](val tracer: Tracer[F]) {
  ...
  val Effect: Kleisli.type = ReaderT
  ...
  implicit val readerTPropagationInstance: Propagation[Effect] =
    new Propagation[Effect] {
      ...
      override def currentSpan(): Effect[Span] = ReaderT.ask[F, Span]

      override def useSpanIn[A](span: Span)(fa: Effect[A]): Effect[A] = ReaderT.local[F, A, Span](_ => span)(fa)
      ...
      override def export(span: Span): Effect[Headers] = ReaderT.liftF(tracer.export(span))

      override def startChild(span: Span, operationName: String): Effect[Span] =
        ReaderT.liftF(tracer.startChild(span, operationName))

      override def finish(span: Span): Effect[Unit] = ReaderT.liftF(tracer.finish(span))
