...
/**
  * Tracing was not in mind when building this algebra but thanks to Tagless Final it could propagate the context
  */
...
trait Console[F[_]] {
  ...
  final def mapK[G[_]](f: F ~> G): Console[G] = a => f(println(a))
}
